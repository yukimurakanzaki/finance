...
import {
  DEFAULT_ASSUMPTIONS,
  assumptionsRepo,
} from '@db/repositories/assumptions.repo'
...
export function AssumptionsEditor() {
  ...
  return (
    ...
    >
      <SectionHeader>FI Target</SectionHeader>
      ...
      <div style={{ fontSize: 'var(--text-caption)', color: 'var(--ink-3)' }}>
        Low: {formatRp(parseNum(targetLow) || 0)} · High:{' '}
        {formatRp(parseNum(targetHigh) || 0)}
      </div>

      <SectionHeader>Annual Return Rates (real, after inflation)</SectionHeader>
      <div style={{ display: 'flex', gap: 'var(--space-3)' }}>
        <div style={{ flex: 1 }}>
          <Field label="RDPU %">
            <Input
              ...
              mono
              value={returnRdpu}
              onChange={(e) => setReturnRdpu(e.target.value)}
              placeholder="3"
            />
          </Field>
        </div>
        <div style={{ flex: 1 }}>
          <Field label="Equity %">
            <Input
              ...
              mono
              value={returnEquity}
              onChange={(e) => setReturnEquity(e.target.value)}
              placeholder="7"
            />
          </Field>
        </div>
        ...
        <div style={{ flex: 1 }}>
          <Field label="DPLK %">
            <Input
              ...
              mono
              value={returnDplk}
              onChange={(e) => setReturnDplk(e.target.value)}
              placeholder="4"
            />
          </Field>
        </div>
      ...
      <div style={{ display: 'flex', gap: 'var(--space-3)' }}>
        ...
        <div style={{ flex: 1 }}>
          <Field label="Equity switch (months)">
            <Input
              type="text"
              inputMode="numeric"
              mono
              value={equitySwitch}
              onChange={(e) => setEquitySwitch(e.target.value)}
              placeholder="6"
            />
          </Field>
        </div>

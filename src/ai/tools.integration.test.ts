...
describe('Write Contract: uses same domain paths', () => {
  ...
  // C1: the tool schema accepts an explicit recurring_item_id from the model,
  // which takes precedence over the description-based auto-match fallback,
  // but only once validated against a real active recurring item.
  it('log_transactions honors an explicit recurring_item_id from the model', async () => {
    const recId = (await db.recurringItems.toCollection().first())!.id!
    const res = JSON.parse(
      await executeWriteTool('log_transactions', {
        transactions: [
          {
    ...
    expect(res.saved_count).toBe(1)
    const txn = await db.transactions.toCollection().first()
    expect(txn?.recurring_item_id).toBe(recId)
  })

  it('log_transactions rejects an unknown recurring_item_id from the model rather than linking it', async () => {
    const res = JSON.parse(
      await executeWriteTool('log_transactions', {
        transactions: [
          {
            ...
            lane: 'protected_living',
            note: 'Kopi',
            recurring_item_id: 'hallucinated-id',
          },
        ],
      }),
    )
    expect(res.saved_count).toBe(1)
    const txn = await db.transactions.toCollection().first()
    expect(txn?.recurring_item_id).toBeNull()
  })

  it('log_transactions rejects a recurring_item_id belonging to an inactive item', async () => {
    const inactiveId = await db.recurringItems.add({
      name: 'Old Gym',
      amount: 200_000,
      cadence: 'monthly',
      kind: 'personal_sub',
  ...
  it('create_account writes to same db.accounts as UI', async () => {
    const res = JSON.parse(
      await executeWriteTool('create_account', {
        name: 'BCA Digital',
        institution: 'BCA',
        account_type: 'bank',
        lane: 'protected_living',
  ...
  it('add_recurring_item writes to same db.recurringItems as UI', async () => {
    const res = JSON.parse(
      await executeWriteTool('add_recurring_item', {
        name: 'Spotify',
        amount: 49_000,
        cadence: 'monthly',
        kind: 'personal_sub',
...
describe('Tool Contract: invalid input rejected before persistence', () => {
  ...
  it('update_asset_value rejects unknown asset id', async () => {
    const res = JSON.parse(
      await executeWriteTool('update_asset_value', {
  ...
  it('delete_memory rejects unknown memory id', async () => {
    const res = JSON.parse(
      await executeWriteTool('delete_memory', { memory_id: 'fake-id' }),
    )
    expect(res.error).toBeDefined()
  })
...
describe('Failure Contract: failed tool call causes no mutation', () => {
  it('unknown write tool returns error, no DB mutation', async () => {
    const res = JSON.parse(
      await executeWriteTool('malicious_tool', { data: 'evil' }),
    )
    expect(res.error).toBeDefined()
